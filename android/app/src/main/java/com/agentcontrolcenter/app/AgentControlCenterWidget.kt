...
/**
 * Agent Control Center Home Screen Widget 2.0
 * 交互式 Widget：状态仪表盘 + 快捷输入 + 最后消息预览
 */
class AgentControlCenterWidget : AppWidgetProvider() {

    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        for (widgetId in appWidgetIds) {
            val views = RemoteViews(context.packageName, R.layout.agent_control_center_widget)
    ...
    private fun updateWidgetData(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val prefs = context.getSharedPreferences("agent_control_center_prefs", Context.MODE_PRIVATE)
    ...
    companion object {
        ...
        fun updateAll(context: Context) {
            ...
            val provider = ComponentName(context, AgentControlCenterWidget::class.java)
