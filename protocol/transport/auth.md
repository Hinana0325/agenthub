# Agent Control Center 认证与令牌规范

本文件定义 Agent Control Center 统一协议层中的认证机制、API Key 存储格式与端到端传输加密方案。认证是所有传输层（HTTP/SSE/WebSocket/MCP）的前置依赖，存储格式与加密方案的跨平台一致性是双端互操作的基础。本规范为 Android（Kotlin）与 iOS（Swift）双端共享的永久契约，`AKS:` 与 `AH1:` 前缀格式必须双端严格一致。

## 1. 认证机制总览

Agent Control Center 根据传输层使用三种认证机制，各机制独立但共用同一 apiKey 凭据：
...
- **加密流程**：
  1. 从 AndroidKeyStore 取出 `agentcontrolcenter_master_key` 对应的 SecretKey。
  2. 随机生成 12 字节 IV。
...
- **密钥保护**：通过 Keychain 的 `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` 等访问控制保护主密钥，绑定设备。
...
Agent Control Center 存在两套加密体系，二者目的、密钥来源、使用场景均不同，不可混淆：
...
| 主密钥别名/标识 | `agentcontrolcenter_master_key` | Keychain item key（等价命名） |
...
| Keychain 访问控制 | N/A | `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` |
| 硬件 backed 标志 | `setUserAuthenticationRequired(false)`（前台服务需后台访问）/ `StrongBox` | Secure Enclave（设备支持时） |
